...
\end{exercise}
\begin{solution}
  Let $f : X \to X$ be an idempotent set-function.
  Define $\alpha : X \to f(X)$ as $\alpha(x) = f(x)$ for each $x \in X$, and define $\beta : f(X) \to X$ as the inclusion map.
  Then, for each $x \in X$, we have $\beta(\alpha(x)) = \beta(f(x)) = f(x)$, which implies that $\beta \circ \alpha = f$.
  Moreover, for each $y \in f(X)$, there exists $x \in X$ such that $f(x) = y$, and thus $f(y) = f(f(x)) = f(x) = y$.
  This immediately implies that $\alpha \circ \beta = 1_{f(X)}$ since $\alpha(\beta(y)) = \alpha(y) = f(y) = y$.
  Therefore, $f$ splits as desired.
\end{solution}
...
\begin{solution}
  ...
  If $f$ has a left inverse $g : b \to a$, then $g \circ f = 1_a$, and thus $f \circ g \circ f = f \circ 1_a = f$.
  If $f$ has a right inverse $h : b \to a$, then $f \circ h = 1_b$, and thus $f \circ h \circ f = 1_b \circ f = f$.

  For the second part, let $f : a \to b$ be a set-function with $a \neq \emptyset$.
  Choose an element $x_0 \in a$.
  Define the function $g : b \to a$ as follows:
  \[ g(y) = \begin{cases}
    x, & \text{if there exists }x \in a \text{ such that } f(x) = y; \\
    x_0, & \text{otherwise}.
  \end{cases} \]
  Then, for each $x \in a$, $g(f(x)) = x$ by the definition of $g$, which implies that $f(g(f(x))) = f(x)$.
  Therefore, we have $f \circ g \circ f = f$, and thus $f$ is regular, as desired.
\end{solution}
...
\end{exercise}
\begin{solution}
  Let $\left\langle X, e, t \right\rangle$ be an object in the given category.
  Defining $f : \mathbb{N} \to X$ as $f(n) = t^n(e)$ for each natural number $n$, where $t^n$ denotes the $n$-th iterate of $t$ (with $t^0$ being the identity function), we have:
  \begin{itemize}
    \item $f(0) = t^0(e) = e$, so $f \, 0 = e$;
    \item for each natural number $n$, $f(n+1) = t^{n+1}(e) = t(t^n(e)) = t(f(n))$, so $f \circ (- + 1) = t \, f$.
  \end{itemize}
  Hence, $f : \left\langle \mathbb{N}, 0, (- + 1) \right\rangle \to \left\langle X, e, t \right\rangle$ is an arrow in the category.
  Moreover, for any other arrow $g : \left\langle \mathbb{N}, 0, (- + 1) \right\rangle \to \left\langle X, e, t \right\rangle$, we can show by induction on $n$ that $g(n) = f(n)$ for each natural number $n$.
  Therefore, $f$ is the unique arrow from the object $\left\langle \mathbb{N}, 0, (- + 1) \right\rangle$ to an object $\left\langle X, e, t \right\rangle$ in the given category, proving that $\left\langle \mathbb{N}, 0, (- + 1) \right\rangle$ is an initial object in the category.
\end{solution}
...
\end{exercise}
\begin{solution}
  Say $f \circ \alpha = f \circ \beta$ for some arrows $\alpha$ and $\beta$ of $C$.
  Then $\alpha$ and $\beta$ are parallel arrows so it suffices to show that $T \, \alpha = T \, \beta$.
  Since $T$ is a functor, we have $T \, (f \circ \alpha) = T \, f \circ T \, \alpha$ and $T \, (f \circ \beta) = T \, f \circ T \, \beta$.
  Thus, we have $T \, f \circ T \, \alpha = T \, f \circ T \, \beta$, and it follows that $T \, \alpha = T \, \beta$ since $T \, f$ is monic.
  Since $T$ is faithful, we conclude that $\alpha = \beta$, and therefore $f$ is monic, as desired.
\end{solution}
...
\end{exercise}
\begin{solution}
  By the fact that $f_i \in U$ for every $i \in I$, the image of $f$ is a subset of $U$.
  Hence, by the condition (v) in the definition of a universe, the image of $f$ is an element of $U$.
  Now, by the condition (iv) in the definition of a universe, the union of the image of $f$ is an element of $U$.
  Thus, by the condition (ii) in the definition of a universe, the cartesian product of $I$ and the union of the image of $f$ is an element of $U$.
  Finally, by the definition of the cartesian product $\prod_i f_i$, we know that $\prod_i f_i$ is a subset of the cartesian product of $I$ and the union of the image of $f$.
  Therefore, combining the conditions (i) and (v) in the definition of a universe, we conclude that $\prod_i f_i$ is an element of $U$.
\end{solution}
...
\end{exercise}
\begin{solution}
  (a) If $f_i = U$ for every $i \in I$, then $\bigcup_i f_i = U$, which is not a set of $U$, obviously.
  Hence, we need to assume that $f_i \in U$ for each $i \in I$, to prove that $\bigcup_i f_i$ is a set of $U$.
  With this assumption, by the condition (v) in the definition of a universe, we know that the image of $f$ is an element of $U$.
  Thus, by the condition (iv) in the definition of a universe, the union of the image of $f$, which is equal to $\bigcup_i f_i$, is an element of $U$.

  (b) Let (vi) denote the closure property given in (a), and assume (vi) holds for any universes.
  Say $f : a \to b$ is a surjective function with $a \in U$ and $b \subset U$.
  Define $g : a \to \bigcup b$ by $g(x) = \left\{ f(x) \right\}$ for each $x \in a$.
  Then, by the conditions (iii) and (i) in the definition of a universe, $g(x) \in U$ for each $x \in a$.
  Hence, by the condition (vi), the union $\bigcup_{x \in a} g(x) = \bigcup_{x \in a} \left\{ f(x) \right\} = b$ is an element of $U$.
  Therefore, we conclude that (vi) can replace condition (v), as desired.
\end{solution}
\end{exercise}
